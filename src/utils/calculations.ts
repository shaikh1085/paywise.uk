...
export function calculateStudentLoanRepayments(
  ...
  config: typeof UK_TAX_CONFIGS['2025_26']
...
export function calculateStudentLoan(input: StudentLoanInput): StudentLoanResult {
  const config = UK_TAX_CONFIGS['2025_26'];
  const { studentLoanAnnual, postgradLoanAnnual } = calculateStudentLoanRepayments(
    input.annualSalary,
    input.plan,
    config
...
export function calculateMinimumWage(input: MinimumWageInput): MinimumWageResult {
  ...
  const applicableMinimumRate = is2024_25 ? bandConfig.rate2024_25 : bandConfig.rate2025_26;
...
export function calculateSickPay(input: SickPayInput): SickPayResult {
  ...
  const config = UK_TAX_CONFIGS[taxYear] || UK_TAX_CONFIGS['2025_26'];
...
export function calculateNationalInsuranceContributions(input: NationalInsuranceInput): NationalInsuranceResult {
  const niYearConfig = UK_NATIONAL_INSURANCE_CONFIGS[input.taxYear] || UK_NATIONAL_INSURANCE_CONFIGS['2025_26'];
...
export function calculateEmployerNationalInsurance(input: EmployerNiInput): EmployerNiResult {
  const niConfig = UK_NATIONAL_INSURANCE_CONFIGS[input.taxYear] || UK_NATIONAL_INSURANCE_CONFIGS['2025_26'];
  const taxConfig = UK_TAX_CONFIGS[input.taxYear] || UK_TAX_CONFIGS['2025_26'];
...
export function calculateDividendTax(input: DividendTaxInput): DividendTaxResult {
  const taxConfig = UK_TAX_CONFIGS[input.taxYear] || UK_TAX_CONFIGS['2025_26'];
  const divConfig = UK_DIVIDEND_TAX_CONFIGS[input.taxYear] || UK_DIVIDEND_TAX_CONFIGS['2025_26'];
...
export function calculateIsaGrowth(input: IsaInput): IsaResult {
  const isaConfig = UK_ISA_CONFIGS[input.taxYear || '2025_26'] || UK_ISA_CONFIGS['2025_26'];
...
export function calculateCapitalGainsTax(input: CgtInput): CgtResult {
  const cgtConfig = UK_CAPITAL_GAINS_TAX_CONFIGS[input.taxYear || '2025_26'] || UK_CAPITAL_GAINS_TAX_CONFIGS['2025_26'];
...
export function calculateStampDuty(input: StampDutyInput): StampDutyResult {
  const configKey = input.taxYearKey || '2025_26';
  const config = UK_STAMP_DUTY_CONFIGS[configKey] || UK_STAMP_DUTY_CONFIGS['2025_26'];
