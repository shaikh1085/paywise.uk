...
Calculated via PayWise UK (https://www.paywiseuk.co.uk/tax-code-calculator)`;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div className="lg:col-span-7 space-y-6">
        {/* Main Decoded Card */}
        <div className="rounded-3xl bg-[#111111] text-white p-6 sm:p-8 border border-[#2A2A2A] shadow-xl relative overflow-hidden space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-mono font-black text-white px-3 py-1 bg-white/10 rounded-lg">
              {result.rawCode}
            </span>
          </div>
          ...
          <div>
            ...
            <div className="text-3xl sm:text-5xl font-black text-white flex items-baseline gap-2 tracking-tight mt-1">
              {result.estimatedPersonalAllowance >= 0 ? (
                <span>£{result.estimatedPersonalAllowance.toLocaleString('en-GB')}</span>
              ) : (
                <span className="text-amber-400">-£{Math.abs(result.estimatedPersonalAllowance).toLocaleString('en-GB')} (Negative)</span>
              )}
              <span className="text-sm font-normal text-slate-400">/ tax year</span>
            </div>
          </div>

          <div className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-1.5">
            <h3 className="text-xs uppercase font-bold text-emerald-400">Official Meaning</h3>
